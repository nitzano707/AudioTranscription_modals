...
async function processAudioChunk(chunk, transcriptionData, currentChunk, totalChunks) {
    ...
    const myprompt = `תמלל את האודיו המצורף בעברית תוך שימוש במידע הבא: 1. תחום הראיון: חינוך. 2. נושא הריאיון: שילוב AI בחינוך. 3. מילות מפתח: ריאיון, בינה מלאכותית, למידה מותאמת אישית, הכשרת מורים. 4. זיהוי דוברים: הריאיון כולל שני דוברים לפחות. השתמש בתוויות "דובר 1" ו"דובר 2" ושמור על עקביות בהפרדה ביניהם. הקפד לזהות מעבר בין דוברים על פי שינויי קול או תוכן. הקפד על דיוק במונחים מקצועיים, וכלול את המונחים המצוינים בהקשרם הנכון`
