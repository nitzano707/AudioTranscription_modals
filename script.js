...
// 🛠️ פונקציות פיצול WAV (WAV, M4A, MP4, OGG)
...
async function splitAudioFileToWavChunks(file, maxChunkSizeBytes) {
    ...
    try {
        ...
        console.error("בעיה בדיקוד קובץ אודיו. הקובץ כנראה פגום או בפורמט לא נתמך (לאחר M4A/MP4/OGG):", e);
...
async function uploadAudio() {
    ...
    const isOGG = fileType.includes('ogg') || fileExtension === 'ogg';



    // חסום קבצים לא נתמכים

    if (!isMP3 && !isWAV && !isM4A && !isMP4 && !isOGG) {

        alert('פורמט קובץ לא נתמך. נא להעלות MP3, WAV, M4A, MP4 או OGG.');
    ...
    try {
            ...
            if (audioFile.size > MAX_CHUNK_SIZE_BYTES) {
        ...
        else if (isWAV || isM4A || isMP4 || isOGG) {
             ...
             if (audioFile.size > MAX_CHUNK_SIZE_BYTES) {
                ...
                if (wavChunks.length === 1) {
                    ...
                    // גיבוי למקרה של M4A/MP4/OGG קטן שלא עבר המרה תקינה
...
function calculateEstimatedTime(audioFile) {
    ...
    // זו הערכה גסה מאוד, אך נשארת כפי שהייתה עם הוספת OGG
    ...
    if (fileType.includes('mp3') || fileExtension === 'mp3') {
        ...
        globalState.estimatedTime = sizeMB * 1.2;
...
async function getSegmentedText(text, prompt) {
    ...
    while (!success && retries < maxRetries) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.warn(`Retry ${retries}/${maxRetries} after error.`);
            ...
            console.error(`Network error on attempt ${retries}:`, error);
    ...
    throw new Error("Failed to get segmented text after maximum retries.");

}



function splitTextIntoSegments(text, maxLength = 4000) {
        ...
        if ((currentSegment + sentence).length > maxLength) {
            ...
            if (sentence.length > maxLength) {
                ...
                while (start < sentence.length) {
                    ...
                    segments.push(sentence.substring(start, start + maxLength));
