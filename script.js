...
// פונקציה לחלוקת קובץ אודיו גדול למקטעים קטנים יותר
async function splitAudioFile(file, maxChunkSizeMB) {
    const chunks = [];
    ...
    if (file.size <= maxChunkSizeBytes) {
        ...
        let start = 0;
        let end = maxChunkSizeBytes;

        while (start < file.size) {
            const chunk = file.slice(start, end);
            chunks.push(chunk);
            start = end;
            end = Math.min(start + maxChunkSizeBytes, file.size);
        }
    }
...
async function uploadAudio() {
    ...
    try {
        ...
        const chunks = await splitAudioFile(audioFile, maxChunkSizeMB);
...
async function processAudioChunk(chunk, transcriptionData, currentChunk, totalChunks) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.segments) {
                let previousEndTime = transcriptionData.length > 0 ? transcriptionData[transcriptionData.length - 1].end : 0;

                // יצירת SRT עבור כל משפט בנפרד והוספת ה-offset המצטבר
                data.segments.forEach((segment, index) => {
                    if (typeof segment.start === 'number' && typeof segment.end === 'number' && segment.start >= previousEndTime) {
                        ...
                        console.warn(`Invalid or non-continuous timestamp for segment ${index}:`, segment);
                    }
                });

                // עדכון ה-offset המצטבר עם הזמן של המקטע האחרון
                const lastSegmentEnd = data.segments[data.segments.length - 1].end;
                cumulativeOffset += lastSegmentEnd;
                console.log(`Updated cumulativeOffset after chunk ${currentChunk}: ${cumulativeOffset}`);
            } else {
...
async function uploadAudio() {
    ...
    if (audioFile.type === 'audio/mpeg') {
        ...
        maxChunkSizeMB = 20;
        console.log("File is not MP3, using default maxChunkSizeMB of 20");
    }
