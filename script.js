...
const OVERLAP_SECONDS = 2; // ✅ חפיפה בין מקטעים (2 שניות)
...
async function uploadAudio() {
    ...
    // בדיקת מגבלת גודל עבור M4A/MP4 (במקום לפצל)
    if ((isM4A || isMP4) && audioFile.size > maxChunkSizeBytes) {
        alert(`קבצי ${isM4A ? 'M4A' : 'MP4'} חייבים להיות קטנים מ-${MAX_SEGMENT_SIZE_MB}MB. המירו ל-MP3 או WAV אם יש צורך.`);
        resetProcess();
        return;
    }
    ...
    try {
        ...
        // === WAV, M4A, MP4 ===
            ...
            if (audioFile.size > maxChunkSizeBytes) {
                // פיצול ל-chunks עם header תקני (decode + encode)
...
async function processAudioChunk(chunk, transcriptionData, currentChunk, totalChunks) {
    ...
    // ✅ הוספת retry אוטומטי עד 3 ניסיונות
    const MAX_ATTEMPTS = 3;
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
            console.log(`Sending chunk ${currentChunk} of ${totalChunks} (attempt ${attempt})...`);
            ...
            if (response.ok) {
                ...
                if (data.segments && data.segments.length > 0) {
                    // ✅ שימוש בזמן יחסי מדויק בתוך המקטע
                    ...
                    // ✅ עדכון זמן מצטבר לפי טווח אמיתי במקטע
                    if (typeof firstSegment?.start === 'number' && typeof lastSegment?.end === 'number') {
                        const delta = lastSegment.end - firstSegment.start;
    ...
    console.error(`Failed to process chunk ${currentChunk} after ${MAX_ATTEMPTS} attempts.`);
}
